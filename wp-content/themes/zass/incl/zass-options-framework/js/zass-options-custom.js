...
jQuery(document).ready(function ($) {
	...
	$('div.import_zass_demo span.of-radio-img-img').on('click', function () {
		if (ask_for_confirmation_flag) {
			$import_true = confirm('Warning! Importing the demo may override some of your existing data. Make sure that all required and recomemended plugins are activated. OK to continue.');
		...
		$action = 'zass_import_' + $option_el.attr('id').substring(15);

		$(this).before('<span id="zass_import_spinner" class="zass_import_spinner"></span>');
		$spinner_el = $("#zass_import_spinner");
		...
		$.post(ajaxurl, data, function (response) {
			...
			import_done_indx = response.search("zass_import_done");
			...
			if (-1 !== import_done_indx) {
				...
				alert('Timeout reached. Data is partialy imported. Please run the import again.');
			}
	...
	$('div.import_zass_demo span.of-radio-img-img').hover(function () {
		if (!$('span.zass_import_spinner').length) {
			$(this).prepend('<span class="zass_import_btn">Import</span>');
		}
	}, function () {
		$(this).find('span.zass_import_btn').remove();
	});
});