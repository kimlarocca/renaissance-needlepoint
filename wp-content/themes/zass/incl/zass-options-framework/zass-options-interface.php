 ...
 */
function zass_optionsframework_tabs() {
	...
	foreach ($options as $value) {
		...
		if ($value['type'] == "heading") {
			$jquery_click_hook = preg_replace('/[^a-zА-Я0-9._\-]/', '', strtolower($value['name']));
 ...
 */
function zass_optionsframework_fields() {
	...
	foreach ($options as $value) {
		...
		switch ($value['type']) {
			...
			case "images":
				...
				foreach ($value['options'] as $key => $option) {
					...
					$output_escaped .= '<div class="of-radio-img-label">' . esc_html($key) . '</div>';

					$output_escaped .= '<span class="of-radio-img-img' . esc_attr($selected) . '" onclick="document.getElementById(\'' . esc_js($value['id'] . '_' . $key) . '\').checked=true;"><img src="' . esc_url($option) . '" /></span>';
				}
				break;
			...
			// Color picker
			case "color":
				$output_escaped .= '<input class="zass-theme-options-colorpicker" name="' . esc_attr($option_name . '[' . $value['id'] . ']') . '" id="' . esc_attr($value['id']) . '" type="text" value="' . esc_attr($val) . '" />';
				break;

			// Uploader
			case "upload":
				$output_escaped .= zass_optionsframework_medialibrary_uploader($value['id'], $val, null);
				break;

			// zass Uploader
			case "zass_upload":
				$is_multiple = array_key_exists('is_multiple', $value) && $value['is_multiple'] ? true : false;
				$output_escaped .= zass_medialibrary_uploader($value['id'], $val, null, '', 0, '', $is_multiple);
				break;
			...
			case 'typography':
				...
				$typography_options = array(
						'sizes' => zass_recognized_font_sizes(),
						'faces' => zass_recognized_font_faces(),
						'styles' => zass_recognized_font_styles(),
						'color' => true,
						'preview' => false
				);
				...
				if ($typography_options['color']) {
					$font_color = '<input class="of-color of-typography of-typography-color zass-theme-options-colorpicker" name="' . esc_attr($option_name . '[' . $value['id'] . '][color]') . '" id="' . esc_attr($value['id'] . '_color') . '" type="text" value="' . esc_attr($typography_stored['color']) . '" />';
				}
				...
				if ($typography_options['preview']) {
					...
					if ($value['id'] == 'body_font') {
						...
						$headings_font = zass_get_option('headings_font', array('face' => 'Oswald'));
					...
					$font_preview = '<div id="' . esc_attr($value['id']) . '_preview" class="zass_font_preview">';
				...
				$typography_fields = apply_filters('zass_typography_fields', $typography_fields, $typography_stored, $option_name, $value);
			...
			case 'background':
				...
				// Background Color
				$output_escaped .= '<input class="of-color of-background of-background-color zass-theme-options-colorpicker" name="' . esc_attr($option_name . '[' . $value['id'] . '][color]') . '" id="' . esc_attr($value['id'] . '_color') . '" type="text" value="' . esc_attr($background['color']) . '" />';

				$output_escaped .= zass_medialibrary_uploader($value['id'], $background['image'], null, '', 0, 'image', false);
				...
				$repeats = zass_recognized_background_repeat();
				...
				$positions = zass_recognized_background_position();
				...
				$attachments = zass_recognized_background_attachment();
			...
			case "info":
				...
				if ($value['desc']) {
					$output_escaped .= apply_filters('zass_sanitize_info', $value['desc']) . "\n";
				}
				$output_escaped .= '</div>' . "\n";
				break;
			...
			case "start_section":
				...
				if (isset($value['name'])) {
					$output_escaped .= '<h4 class="acc-heading"><strong>' . esc_html($value['name']) . '</strong></h4>' . "\n";
				}
			...
			case 'sidebar':
				...
				$output_escaped .= '<input type="button" value="' . esc_html__('Add', 'zass') . '" class="button" id="add_custom_sidebar">';
				...
				if (count($custom_sidebars)) {
					$output_escaped .= '<h4>' . esc_html__('List of created custom sidebars:', 'zass') . '</h4>';

					$output_escaped .= '<div id="zass_custom_sidebars_list">';

					foreach ($custom_sidebars as $sidebar) {
						$output_escaped .= '<div><a class="zass_del_sdbr" data-sidebar-name="' . $sidebar . '" href="#"></a> ' . $sidebar . '</div>';
					}

					$output_escaped .= '</div>';
				} else {
					$output_escaped .= '<h4 class="zass_no_custom_sidebars">' . esc_html__('No custom sidebars.', 'zass') . '</h4>';
					$output_escaped .= '<div id="zass_custom_sidebars_list"></div>';
				}
		...
		// This variable has been safely escaped in the following file: zass/incl/zass-options-framework/zass-options-interface.php Line: 56 - 488
