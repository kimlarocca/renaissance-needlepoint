 ...
 */
if (!class_exists('Zass_WXR_Parser')) {

	class Zass_WXR_Parser {

		function parse($file) {
			// Attempt to use proper XML parsers first
			if (extension_loaded('simplexml')) {
				$parser = new Zass_WXR_Parser_SimpleXML;
			...
			if (isset($result) && defined('IMPORT_DEBUG') && IMPORT_DEBUG) {
				...
				echo '</pre>';
				echo '<p><strong>' . esc_html__('There was an error when reading this WXR file', 'zass-plugin') . '</strong><br />';
				echo esc_html__('Details are shown above. The importer will now try again with a different parser...', 'zass-plugin') . '</p>';
			}
 ...
 */
if (!class_exists('Zass_WXR_Parser_SimpleXML')) {

	class Zass_WXR_Parser_SimpleXML {
		...
		function parse($file) {
			...
			if (!$success || isset($dom->doctype)) {
				return new WP_Error('SimpleXML_parse_error', esc_html__('There was an error when reading this WXR file', 'zass-plugin'), libxml_get_errors());
			}
			...
			if (!$wxr_version)
				return new WP_Error('WXR_parse_error', esc_html__('This does not appear to be a WXR file, missing/invalid WXR version number', 'zass-plugin'));
